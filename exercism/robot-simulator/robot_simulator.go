...
func Room(extent Rect, robot Step2Robot, act chan Action, rep chan Step2Robot) {
	for {
		...
		// establish if the robot can move
...
func StartRobot3(name, script string, action chan Action3, log chan string) {
	...
	// script ended so send finish
	action <- Action3{F3, name}
}
...
func Room3(
	...
	log chan string) {
	defer close(report)
	actions := map[rune]bool{I3: true, L3: true, A3: true, R3: true, F3: true}
	// map from name to robot
	var names = make(map[string]*Step3Robot)
	// map to use as a set to check for uniqueness
	var initial_positions = make(map[Pos]int)
	// check the initial setup is valid
	...
	for {
		...
		cmd, ok := map[rune]Command{
			I3: I, L3: L, A3: A, R3: R, F3: I,
		}[what.action]
		if !ok {
